...
from Embeddings import LayerNormalization
from Encoder_Layer import CNN


class DecoderLayer(NNModule):
    def __init__(self, hidden_size, num_heads, d_model, conv_hidden_dim, p, epl=1e-6, layer_name="DecL"):
        super().__init__()
    ...
    def forward(self, Henc, Y, mask=None):
        ...
        attn_output, attn_weights = self.forward(Y, mask) # (seq_length, self.d_model)
        ...
        # Feed forward
        cnn_output = self.cnn.forward(out2 + cnn_output) # (seq_length, self.d_model)

        # Second layer norm after adding residual connection
        out3 = self.layernorm2.forward(attn_weights_cross) # (seq_length, self.d_model)
